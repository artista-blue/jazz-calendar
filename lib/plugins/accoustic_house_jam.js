(function () {
	...
	client.fetch(getScheduleURL(year, month), args, function (err, $, res) {
		...
		if (isDR[0] === true) {
		    ...
		    let prefix = '';
		    let offset = 0;
		    if (children.length !== dateArray.length) {
			if (isFirstRow === true) {
			    offset = dateArray.length -  children.length;
			} else {
			    //prefix = "!!!!!";
			}
		    }
			...
			text = text.replace(/\n/g, '');
			text = text.replace(/ +/g, ' ');
			const dayOfMonth = dateArray[idx + offset];
			if (dayOfMonth === null) {
			    return;
			}
			schedule[year][month][dayOfMonth] = prefix + text;
		    });
		    dateArray = undefined;
		    isFirstRow = false;
		}
